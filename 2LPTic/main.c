...
void displacement_fields(void)
{
...
#ifdef PARTICLENU
	double delta_nu, phase_nu, ampl_nu, p_of_k_nu, maxdisp_nu, max_disp_glob_nu;
	fftw_complex *(cdisp_nu[3]), *(cdisp2_nu[3]); /* ZA and 2nd order displacements */
	fftw_real *(disp_nu[3]), *(disp2_nu[3]);

	fftw_complex *(cdigrad_nu[6]);
	fftw_real *(digrad_nu[6]);
#endif
...
#ifdef PARTICLENU
		cdisp_nu[axes] = (fftw_complex *)malloc(bytes += sizeof(fftw_real) * TotalSizePlusAdditional);
		disp_nu[axes] = (fftw_real *)cdisp_nu[axes];
#endif
	}

	ASSERT_ALLOC(cdisp[0] && cdisp[1] && cdisp[2]);
#ifdef PARTICLENU
	ASSERT_ALLOC(cdisp_nu[0] && cdisp_nu[1] && cdisp_nu[2]);
#endif
...
#endif
	{
...
#ifdef PARTICLENU
						cdisp_nu[axes][(i * Nmesh + j) * (Nmesh / 2 + 1) + k].re = 0;
						cdisp_nu[axes][(i * Nmesh + j) * (Nmesh / 2 + 1) + k].im = 0;
#endif
					}
			...
			if ((i >= Local_x_start && i < (Local_x_start + Local_nx)) || (ii >= Local_x_start && ii < (Local_x_start + Local_nx)))
			{
				for (j = 0; j < Nmesh; j++)
				{
					...
					for (k = 0; k < Nmesh / 2; k++)
					{
						...
						while (ampl == 0);
#ifdef PARTICLENU
						phase_nu = gsl_rng_uniform(random_generator) * 2 * PI;
						do
							ampl_nu = gsl_rng_uniform(random_generator);
						while (ampl_nu == 0);
#endif
						...
						else
						{
							...
							if (fabs(kvec[2]) * Box / (2 * PI) > Nsample / 2)
								continue;
#ifdef PARTICLENU
							if (fabs(kvec[0]) * Box / (2 * PI) > NmeshNu / 2)
								continue;
							if (fabs(kvec[1]) * Box / (2 * PI) > NmeshNu / 2)
								continue;
							if (fabs(kvec[2]) * Box / (2 * PI) > NmeshNu / 2)
								continue;
#endif
						}
						...
						{
#ifdef PARTICLENU
							p_of_k_nu = PowerSpec_nu(kmag) * (-log(ampl_nu));
							delta_nu = fac * sqrt(p_of_k_nu);
#endif
...
#ifdef PARTICLENU
									cdisp_nu[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].re = -kvec[axes] / kmag2 * delta_nu * sin(phase_nu);
									cdisp_nu[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].im = kvec[axes] / kmag2 * delta_nu * cos(phase_nu);
#endif
								}
						}
...
#ifdef PARTICLENU
											cdisp[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].re = -kvec[axes] / kmag2 * delta_nu * sin(phase_nu);
											cdisp[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].im = kvec[axes] / kmag2 * delta_nu * cos(phase_nu);

											cdisp[axes][((i - Local_x_start) * Nmesh + jj) * (Nmesh / 2 + 1) + k].re = -kvec[axes] / kmag2 * delta_nu * sin(phase_nu);
											cdisp[axes][((i - Local_x_start) * Nmesh + jj) * (Nmesh / 2 + 1) + k].im = -kvec[axes] / kmag2 * delta_nu * cos(phase_nu);
#endif
										}
									}
								}
...
#ifdef PARTICLENU
											cdisp_nu[axes][((ii - Local_x_start) * Nmesh + jj) * (Nmesh / 2 + 1) + k].re = -kvec[axes] / kmag2 * delta_nu * sin(phase_nu);
											cdisp_nu[axes][((ii - Local_x_start) * Nmesh + jj) * (Nmesh / 2 + 1) + k].im = -kvec[axes] / kmag2 * delta_nu * cos(phase_nu);
#endif
										}
								}
							}
...
#ifdef PARTICLENU
			cdigrad_nu[i] = (fftw_complex *)malloc(bytes = sizeof(fftw_real) * TotalSizePlusAdditional);
			digrad_nu[i] = (fftw_real *)cdigrad_nu[i];
			ASSERT_ALLOC(cdigrad_nu[i]);
#endif 
		}
...
#ifdef PARTICLENU
					/* Same for neutrino particles. */
					cdigrad_nu[0][coord].re = -cdisp_nu[0][coord].im * kvec[0]; /* disp0,0 */
					cdigrad_nu[0][coord].im = cdisp_nu[0][coord].re * kvec[0];

					cdigrad_nu[1][coord].re = -cdisp_nu[0][coord].im * kvec[1]; /* disp0,1 */
					cdigrad_nu[1][coord].im = cdisp_nu[0][coord].re * kvec[1];

					cdigrad_nu[2][coord].re = -cdisp_nu[0][coord].im * kvec[2]; /* disp0,2 */
					cdigrad_nu[2][coord].im = cdisp_nu[0][coord].re * kvec[2];

					cdigrad_nu[3][coord].re = -cdisp_nu[1][coord].im * kvec[1]; /* disp1,1 */
					cdigrad_nu[3][coord].im = cdisp_nu[1][coord].re * kvec[1];

					cdigrad_nu[4][coord].re = -cdisp_nu[1][coord].im * kvec[2]; /* disp1,2 */
					cdigrad_nu[4][coord].im = cdisp_nu[1][coord].re * kvec[2];

					cdigrad_nu[5][coord].re = -cdisp_nu[2][coord].im * kvec[2]; /* disp2,2 */
					cdigrad_nu[5][coord].im = cdisp_nu[2][coord].re * kvec[2];
#endif
				}

		if (ThisTask == 0)
			printf("Fourier transforming displacement gradient...");
		for (i = 0; i < 6; i++)
			rfftwnd_mpi(Inverse_plan, 1, digrad[i], Workspace, FFTW_NORMAL_ORDER);
#ifdef PARTICLENU
		for (i = 0; i < 6; i++)
			rfftwnd_mpi(Inverse_plan, 1, digrad_nu[i], Workspace, FFTW_NORMAL_ORDER);
#endif
		if (ThisTask == 0)
			printf("Done.\n");
		...
		for (i = 0; i < Local_nx; i++)
			for (j = 0; j < Nmesh; j++)
				for (k = 0; k < Nmesh; k++)
				{
					...
					digrad[3][coord] = digrad[0][coord] * (digrad[3][coord] + digrad[5][coord]) + digrad[3][coord] * digrad[5][coord] - digrad[1][coord] * digrad[1][coord] - digrad[2][coord] * digrad[2][coord] - digrad[4][coord] * digrad[4][coord];
#ifdef PARTICLENU
					digrad_nu[3][coord] = digrad_nu[0][coord] * (digrad_nu[3][coord] + digrad_nu[5][coord]) + digrad_nu[3][coord] * digrad_nu[5][coord] - digrad_nu[1][coord] * digrad_nu[1][coord] - digrad_nu[2][coord] * digrad_nu[2][coord] - digrad_nu[4][coord] * digrad_nu[4][coord];
#endif
				}
...
#ifdef PARTICLENU
		rfftwnd_mpi(Forward_plan, 1, digrad_nu[3], Workspace, FFTW_NORMAL_ORDER);
#endif
		if (ThisTask == 0)
			printf("Done.\n");
...
#ifdef PARTICLENU
			cdisp2_nu[axes] = cdigrad_nu[axes];
			disp2_nu[axes] = (fftw_real *)cdisp2_nu[axes];
#endif
		}
...
#ifdef PARTICLENU
							cdisp2_nu[axes][coord].re = cdigrad_nu[3][coord].im * kvec[axes] / kmag2;
							cdisp2_nu[axes][coord].im = -cdigrad_nu[3][coord].re * kvec[axes] / kmag2;
#endif
						}
						else
						{
...
#ifdef PARTICLENU
							cdisp2_nu[axes][coord].re = cdisp2_nu[axes][coord].im = 0.0;
#endif
						}
#ifdef CORRECT_CIC
...
#ifdef PARTICLENU
						cdisp_nu[axes][coord].re *= smth;
						cdisp_nu[axes][coord].im *= smth;
						cdisp2_nu[axes][coord].re *= smth;
						cdisp2_nu[axes][coord].im *= smth;
#endif
#endif
					}
				}
		...
		{
			if (ThisTask == 0)
				printf("Fourier transforming displacements, axis %d.\n", axes);
...
#ifdef PARTICLENU
			rfftwnd_mpi(Inverse_plan, 1, disp_nu[axes], Workspace, FFTW_NORMAL_ORDER);
			rfftwnd_mpi(Inverse_plan, 1, disp2_nu[axes], Workspace, FFTW_NORMAL_ORDER);
#endif
			...
			if (Local_nx > 0)
			{
				...
				MPI_Wait(&request, &status);
#ifdef PARTICLENU
				MPI_Isend(&(disp_nu[axes][0]),
									sizeof(fftw_real) * Nmesh * (2 * (Nmesh / 2 + 1)),
									MPI_BYTE, recvTask, 10, MPI_COMM_WORLD, &request);

				MPI_Recv(&(disp_nu[axes][(Local_nx * Nmesh) * (2 * (Nmesh / 2 + 1))]),
								 sizeof(fftw_real) * Nmesh * (2 * (Nmesh / 2 + 1)),
								 MPI_BYTE, sendTask, 10, MPI_COMM_WORLD, &status);
				...
				/* send 2nd order disp */
				MPI_Isend(&(disp2_nu[axes][0]),
									sizeof(fftw_real) * Nmesh * (2 * (Nmesh / 2 + 1)),
									MPI_BYTE, recvTask, 10, MPI_COMM_WORLD, &request);

				MPI_Recv(&(disp2_nu[axes][(Local_nx * Nmesh) * (2 * (Nmesh / 2 + 1))]),
								 sizeof(fftw_real) * Nmesh * (2 * (Nmesh / 2 + 1)),
								 MPI_BYTE, sendTask, 10, MPI_COMM_WORLD, &status);
		...
		for (n = 0; n < NumPart; n++)
		{
...
#endif
			{
				...
				for (axes = 0; axes < 3; axes++)
				{
					...
					if (P[n].Type == 2)
					{
						dis = disp_nu[axes][(i * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + k] * f1 +
									disp_nu[axes][(i * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + kk] * f2 +
									disp_nu[axes][(i * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + k] * f3 +
									disp_nu[axes][(i * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + kk] * f4 +
									disp_nu[axes][(ii * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + k] * f5 +
									disp_nu[axes][(ii * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + kk] * f6 +
									disp_nu[axes][(ii * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + k] * f7 +
									disp_nu[axes][(ii * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + kk] * f8;
	
						dis2 = disp2_nu[axes][(i * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + k] * f1 +
									 disp2_nu[axes][(i * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + kk] * f2 +
									 disp2_nu[axes][(i * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + k] * f3 +
									 disp2_nu[axes][(i * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + kk] * f4 +
									 disp2_nu[axes][(ii * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + k] * f5 +
									 disp2_nu[axes][(ii * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + kk] * f6 +
									 disp2_nu[axes][(ii * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + k] * f7 +
									 disp2_nu[axes][(ii * Nmesh + jj) * (2 * (Nmesh / 2 + 1)) + kk] * f8;
						dis2 /= (float)nmesh3;
					}
					else
#endif
...
#ifdef PARTICLENU
	MPI_Reduce(&maxdisp_nu, &max_disp_glob_nu, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
#endif
...
#ifdef PARTICLENU
		printf("\nMaximum displacement for neutrinos: %g kpc/h, in units of the part-spacing= %g\n", max_disp_glob_nu, max_disp_glob_nu / (Box / Nmesh));
#endif
	}
}
