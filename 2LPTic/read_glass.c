...
#define SKIP2                              \
	{                                        \
		my_fread(&dummy2, sizeof(int), 1, fd); \
	}
	...
	if (ThisTask == 0)
	{
		...
		for (num = 0, skip = 0; num < numfiles; num++)
		{
			...
			if (num == 0)
			{
				...
				/* Current stage: Hard code to give the second type. */
				printf(
	     		"Type 0 (gas):   %8d  (tot=%6d%09d)\n"
	     		"Type 1 (halo):  %8d  (tot=%6d%09d)\n"
	     		"Type 2 (disk):  %8d  (tot=%6d%09d)\n"
	     		"Type 3 (bulge): %8d  (tot=%6d%09d)\n"
	     		"Type 4 (stars): %8d  (tot=%6d%09d)\n"
	     		"Type 5 (bndry): %8d  (tot=%6d%09d)\n\n",
					header1.npart[0], (int) (header1.npartTotal[0] / 1000000000),
	     		(int) (header1.npartTotal[0] % 1000000000), header1.npart[1],
	     		(int) (header1.npartTotal[1] / 1000000000), (int) (header1.npartTotal[1] % 1000000000), 
					header1.npart[2], (int) (header1.npartTotal[2] / 1000000000),
	     		(int) (header1.npartTotal[2] % 1000000000), header1.npart[3],
	     		(int) (header1.npartTotal[3] / 1000000000), (int) (header1.npartTotal[3] % 1000000000),
	     		header1.npart[4], (int) (header1.npartTotal[4] / 1000000000),
	     		(int) (header1.npartTotal[4] % 1000000000), header1.npart[5],
	     		(int) (header1.npartTotal[5] / 1000000000), (int) (header1.npartTotal[5] % 1000000000));
      	fflush(stdout);
#ifdef PARTICLENU
				if(header1.npartTotal[2] != 0) header1.npartTotal[2] = pow(NmeshNu, 3);
#endif
				printf("\nNglass= %d\n\n", Nglass);
...
#ifdef PARTICLENU
	int GlassTileFacNu = GlassTileFac * (NmeshNu / Nsample);
	for (i = 0; i < GlassTileFacNu; i++)
		for (j = 0; j < GlassTileFacNu; j++)
			for (k = 0; k < GlassTileFacNu; k++)
			{
				for (type = 0, n = 0; type < 6; type++)
				{
					for (m = 0; m < header1.npartTotal[type]; m++, n++)
					{
						x = pos[3 * n] / header1.BoxSize * (Box / GlassTileFacNu) + i * (Box / GlassTileFacNu);
	...
	if (ThisTask == 0)
	{
		...
		printf("Total DM particle = %d%09d\n", (int)(pow(Nsample, 3) / 1000000000), (int)(pow(Nsample, 3) % 1000000000));
		printf("Total DM particle = %d%09d\n\n", (int)(pow(NmeshNu, 3) / 1000000000), (int)(pow(NmeshNu, 3) % 1000000000));
		fflush(stdout);
	}
	free(npart_Task);
...
/* Initialize the particle position for neutrinos. */
#ifdef PARTICLENU
	for (i = 0; i < GlassTileFacNu; i++)
		for (j = 0; j < GlassTileFacNu; j++)
			for (k = 0; k < GlassTileFacNu; k++)
			{
				for (type = 0, n = 0; type < 6; type++)
				{
					for (m = 0; m < header1.npartTotal[type]; m++, n++)
					{
						...
						if (Slab_to_task[slab] == ThisTask)
						{
							y = pos[3 * n + 1] / header1.BoxSize * (Box / GlassTileFacNu) + j * (Box / GlassTileFacNu);
							z = pos[3 * n + 2] / header1.BoxSize * (Box / GlassTileFacNu) + k * (Box / GlassTileFacNu);
