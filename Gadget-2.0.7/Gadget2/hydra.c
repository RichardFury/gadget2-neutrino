          ...
          if(All.expan_on == 1)
		    {
	          ...
	          for(i = 0; i < All.NNeutrino; i++)
			    {
		          roneu += neutrino_integration(All.Time, All.mass[i], All.xi[i], i);
                }
	        }
          else
		    {
#ifdef STERILE
			  roneu = neutrino_integration(All.Time, 0., 0., -1) * (All.NNeutrino + All.neff - 4.046);
#else
              roneu = neutrino_integration(All.Time, 0., 0., -1) * All.NNeutrino;
#endif
            }
	      hubble_a = All.Omega0 / (All.Time * All.Time * All.Time) 
	  	    + (1 - All.Omega0 - All.OmegaLambda - All.Omega_nu0_expan) / (All.Time * All.Time)  + All.OmegaLambda + roneu;
	    }
      else
        {
  	      hubble_a = All.Omega0 / (All.Time * All.Time * All.Time)
